...
def _format_image(image_url: str) -> Dict:
    ...
    if match is None:
        raise ValueError(
            "Only base64-encoded images are supported currently."
        )
    return {
        "type": "base64",
...
def _create_usage_metadata() -> UsageMetadata:
    # Return empty usage metadata if the endpoint doesn't provide it.
...
class ChatAnthropicCustom(BaseChatModel):
    ...
    # Custom Bedrock-compatible endpoint URL and API key.
    custom_api_url: str = Field(default_factory=from_env(["CUSTOM_API_URL"], default=""))
    custom_api_key: str = Field(default_factory=from_env(["CUSTOM_API_KEY"], default=""))
